"""Tests for termweave.config module."""
...
class TestConfigEnvironment:
    ...
    def test_host_from_env(self, clean_env, mock_secret_key, monkeypatch):
        """Host should be loaded from TERMWEAVE_HOST."""
        monkeypatch.setenv("TERMWEAVE_HOST", "127.0.0.1")
        config = Config.load()
        assert config.host == "127.0.0.1"

    def test_port_from_env(self, clean_env, mock_secret_key, monkeypatch):
        """Port should be loaded from TERMWEAVE_PORT."""
        monkeypatch.setenv("TERMWEAVE_PORT", "9000")
        config = Config.load()
        assert config.port == 9000

    def test_secret_key_from_env(self, clean_env, monkeypatch):
        """Secret key should be loaded from TERMWEAVE_SECRET_KEY."""
    ...
    def test_passphrase_hash_from_env(self, clean_env, mock_secret_key, monkeypatch):
        """Passphrase hash should be loaded from TERMWEAVE_PASSPHRASE_HASH."""
    ...
    def test_access_token_expire_from_env(self, clean_env, mock_secret_key, monkeypatch):
        """Access token expiry should be loaded from TERMWEAVE_ACCESS_TOKEN_EXPIRE_MINUTES."""
        monkeypatch.setenv("TERMWEAVE_ACCESS_TOKEN_EXPIRE_MINUTES", "30")
        config = Config.load()
        assert config.access_token_expire_minutes == 30

    def test_refresh_token_expire_from_env(self, clean_env, mock_secret_key, monkeypatch):
        """Refresh token expiry should be loaded from TERMWEAVE_REFRESH_TOKEN_EXPIRE_DAYS."""
        monkeypatch.setenv("TERMWEAVE_REFRESH_TOKEN_EXPIRE_DAYS", "14")
        config = Config.load()
...
""")
        monkeypatch.setenv("TERMWEAVE_CONFIG", str(config_file))
        config = Config.load()
...
""")
        monkeypatch.setenv("TERMWEAVE_CONFIG", str(config_file))
        monkeypatch.setenv("TERMWEAVE_PORT", "5000")
        config = Config.load()
...
class TestConfigValidation:
    ...
    def test_secret_key_too_short_raises_error(self, clean_env, monkeypatch):
        ...
        monkeypatch.setenv("TERMWEAVE_SECRET_KEY", "short")
    ...
    def test_invalid_port_raises_error(self, clean_env, mock_secret_key, monkeypatch):
        ...
        monkeypatch.setenv("TERMWEAVE_PORT", "not-a-number")
    ...
    def test_port_out_of_range_raises_error(self, clean_env, mock_secret_key, monkeypatch):
        ...
        monkeypatch.setenv("TERMWEAVE_PORT", "99999")
        with pytest.raises(ConfigError, match="port"):
            Config.load()
    ...
    def test_validate_returns_errors_list(self, clean_env, mock_secret_key, monkeypatch):
        ...
        monkeypatch.setenv("TERMWEAVE_PORT", "-1")
...
class TestConfigProperties:
    ...
    def test_config_dir_property(self, clean_env, mock_secret_key):
        """config_dir property should return Path to ~/.termweave."""
        config = Config.load()
        expected = Path.home() / ".termweave"
...
class TestConfigOptionalFields:
    ...
    def test_debug_mode_from_env(self, clean_env, mock_secret_key, monkeypatch):
        ...
        monkeypatch.setenv("TERMWEAVE_DEBUG", "true")
        config = Config.load()
    ...
    def test_debug_mode_various_truthy_values(self, clean_env, mock_secret_key, monkeypatch):
        ...
        for value in ["true", "True", "TRUE", "1", "yes"]:
            monkeypatch.setenv("TERMWEAVE_DEBUG", value)
            config = Config.load()
